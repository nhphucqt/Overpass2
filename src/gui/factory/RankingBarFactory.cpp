...
RectangleView::Ptr RankingBarFactory::create(Activity* context, sf::Font& font, std::string username, int score, sf::Vector2f size, int fontSize) {
    RectangleView::Ptr barView = std::make_unique<RectangleView>(size, sf::Vector2f(0, 0), sf::Color::Transparent);
    
    TextView::Ptr usernameView = std::make_unique<TextView>(username, font, sf::Vector2f(0, 0), fontSize, sf::Color::White);
    usernameView->setPosition(sf::Vector2f(10, (size.y - usernameView->getGlobalBounds().getSize().y) / 2.f));

    TextView::Ptr scoreView = std::make_unique<TextView>(std::to_string(score), font, sf::Vector2f(0, 0), fontSize, sf::Color::White);
    ...
    barView->setOnMouseMoved(context, [&](EventListener* listener, const sf::Event& event) {
